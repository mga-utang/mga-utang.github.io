...
import { Search, Plus, User, LogOut, Store, AlertCircle, Edit3 } from 'lucide-react'
...
const Dashboard: React.FC = () => {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      alert(t('dash.profile_update_failed'))
    } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {showEditProfile && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end justify-center z-50">
          <div className="bg-white w-full max-w-md rounded-t-2xl p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4">{t('dash.edit_profile_title')}</h2>
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:opacity-50"
                >
                  {loading ? t('cp.saving') : t('dash.save_profile')}
                </button>
              </div>
            </form>
          </div>
